...
/*Table structure contains pointers to pages of raws and keeps
  tracks of how many row there are.	
*/
typedef struct {
	uint32_t num_rows;
	Pager* pager;
...
typedef struct{
	StatementType type;
	Row row_to_insert;		// only be used by insert operation
}Statement;
...
/*
Handle cache miss. Assume that pages are saved one after the other in the
database file: Page 0 at offset 0,Page 1 at 4096 etc. If the requested page
lies outside the bounds of file, we just allocate some memory and return it.
...
// convert to or from the compact representation
...
// To make sure that the input string will not cause buffer overflow
...
// simple compilation of SQL simulate
...
and also intialize the page cache to all NULL
*/
...
2. Initializing a page data structure
